...
def analysis_classification_report_and_cm(all_labels, all_preds, target_names):
    ...
    with open(path_txt, "w") as f:
        f.write("Test Set Performance Analysis (Fusion Multistream CNN)\n")
        f.write("=" * 60 + "\n\n")
        f.write(report)
    ...
    plt.title("Confusion Matrix: Fusion Multistream CNN — BFRB vs. Non-BFRB Gestures")
...
def analysis_per_class_metrics(all_labels, all_preds, target_names):
    ...
    ax.set_xlabel("Score")
    ax.set_title("Per-Class Precision, Recall & F1 (sorted by F1) — Fusion Multistream CNN")
    ax.legend()
    plt.tight_layout()
...
def analysis_confidence(all_outputs, all_labels, all_preds, target_names):
    ...
    axes[0].set_title("Prediction Confidence: Correct vs. Wrong — Fusion Multistream CNN")
    axes[0].legend()
...
def analysis_region_accuracy(all_labels, all_preds, target_names):
    ...
    ax.set_title("Accuracy Grouped by Body Region — Fusion Multistream CNN")
...
def analysis_roc_curves(all_labels, all_probs, target_names):
    ...
    plt.suptitle("One-vs-Rest ROC Curves per Gesture Class — Fusion Multistream CNN", fontsize=12, y=1.01)
    plt.tight_layout()
...
# Main
# ---------------------------------------------------------------------------
def main(checkpoint_path=FUSION_CHECKPOINT_PATH):
