"""
Analyze Multistream CNN (thermal only) trained by train_multistream_cnn_thermal.py.

Loads models/deep_learning_models/Multistream_CNN_thermal_only.pth, rebuilds the same
train/val/test split and label encoding, evaluates on the full test set (no sequences
dropped — imputation fills missing thermal with train mean), and writes reports, figures,
and predictions under analysis_results/multistream_cnn_thermal_only/.
"""
import os
import numpy as np
import pandas as pd
...
from train_multistream_cnn_thermal import (
    ...
    make_thermal_collate_fn,
    get_thm_columns,
    compute_train_thermal_zscore_params,
    DROPOUT_P,
)

CHECKPOINT_PATH = "models/deep_learning_models/Multistream_CNN_thermal_only.pth"
OUTPUT_DIR = "analysis_results/multistream_cnn_thermal_only"
MODEL_LABEL = "Multistream CNN (Thermal Only)"
...
def load_model_and_data(checkpoint_path=CHECKPOINT_PATH):
    """Load checkpoint, rebuild split (full test set, no drop), build model."""
    ...
    print(
        f"Test set (all sequences, no drop): {len(testset_df)} rows, "
    ...
    if isinstance(ckpt, dict) and "model_state_dict" in ckpt:
        ...
        thm_mean = np.asarray(ckpt["thm_mean"], dtype=np.float32) if "thm_mean" in ckpt else None
        thm_std = np.asarray(ckpt["thm_std"], dtype=np.float32) if "thm_std" in ckpt else None
    else:
        sd = ckpt
        nc = num_classes
        dropout_p = DROPOUT_P
    ...
    if thm_mean is None or thm_std is None:
        if "norm.mean" in sd:
            thm_mean = sd["norm.mean"].detach().cpu().numpy().flatten()
            thm_std = sd["norm.std"].detach().cpu().numpy().flatten()
        else:
            thm_mean, thm_std = compute_train_thermal_zscore_params(trainset_df, thm_cols)
            print("Checkpoint missing thermal z-score metadata; computed from train split.")

    model = MultistreamCNNThermalNet(
        num_classes=nc, mean=thm_mean, std=thm_std, dropout_p=dropout_p
    )
    model.load_state_dict(sd, strict=True)
    model = model.to(device)
    model.eval()

    train_mean_raw = thm_mean
...
def analysis_classification_report_and_cm(all_labels, all_preds, target_names):
    ...
    with open(path_txt, "w") as f:
        f.write(f"Test Set Performance Analysis ({MODEL_LABEL})\n")
        f.write("=" * 60 + "\n\n")
        f.write(report)
    ...
    plt.title(f"Confusion Matrix: {MODEL_LABEL} — Gesture Classification")
...
def analysis_per_class_metrics(all_labels, all_preds, target_names):
    ...
    ax.set_xlabel("Score")
    ax.set_title(f"Per-Class Precision, Recall & F1 (sorted by F1) — {MODEL_LABEL}")
    ax.legend()
    plt.tight_layout()
...
def analysis_confidence(all_outputs, all_labels, all_preds, target_names):
    ...
    axes[0].set_title(f"Prediction Confidence: Correct vs. Wrong — {MODEL_LABEL}")
    axes[0].legend()
...
def analysis_region_accuracy(all_labels, all_preds, target_names):
    ...
    ax.set_title(f"Accuracy Grouped by Body Region — {MODEL_LABEL}")
...
def analysis_roc_curves(all_labels, all_probs, target_names):
    ...
    plt.suptitle(f"One-vs-Rest ROC Curves per Gesture Class — {MODEL_LABEL}", fontsize=12, y=1.01)
    plt.tight_layout()
