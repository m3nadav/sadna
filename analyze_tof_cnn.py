...
Test set: full subject-based test split — no sequences dropped (including fully missing ToF).
"""
import os
import numpy as np
import pandas as pd
...
def load_model_and_data(checkpoint_path=CHECKPOINT_PATH):
    """Load checkpoint (full or state_dict only), rebuild split; use entire test set (no ToF drop)."""
