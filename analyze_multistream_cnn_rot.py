"""
Analyze Multistream_CNN_rot_only model: run the same evaluations as for the acc model
and save all analysis results (figures, CSV, text) to analysis_results/multistream_cnn_rot.
Test set uses only sequences with complete quaternion data (same as training).
"""
import os
import numpy as np
import pandas as pd
...
def load_model_and_data(checkpoint_path=CHECKPOINT_PATH):
    """Load checkpoint, rebuild split and label encoding, drop test sequences missing quaternions."""
    ...
    if "model_state_dict" in ckpt:
        model = MultistreamCNNRotNet(num_classes=ckpt["num_classes"])
    ...
    test_ds = RotSequenceDataset(testset_df)
...
def analysis_classification_report_and_cm(all_labels, all_preds, target_names):
    ...
    with open(path_txt, "w") as f:
        f.write("Test Set Performance Analysis (Multistream CNN rot-only)\n")
        f.write("=" * 60 + "\n\n")
        f.write(report)
    ...
    plt.title("Confusion Matrix: Multistream CNN (rot-only) — BFRB vs. Non-BFRB Gestures")
...
def analysis_per_class_metrics(all_labels, all_preds, target_names):
    ...
    ax.set_xlabel("Score")
    ax.set_title("Per-Class Precision, Recall & F1 (sorted by F1) — Multistream CNN (rot-only)")
    ax.legend()
    plt.tight_layout()
...
def analysis_confidence(all_outputs, all_labels, all_preds, target_names):
    ...
    axes[0].set_title("Prediction Confidence: Correct vs. Wrong — Multistream CNN (rot-only)")
    axes[0].legend()
...
def analysis_region_accuracy(all_labels, all_preds, target_names):
    ...
    ax.set_title("Accuracy Grouped by Body Region — Multistream CNN (rot-only)")
...
def analysis_roc_curves(all_labels, all_probs, target_names):
    ...
    plt.suptitle("One-vs-Rest ROC Curves per Gesture Class — Multistream CNN (rot-only)", fontsize=12, y=1.01)
    plt.tight_layout()
