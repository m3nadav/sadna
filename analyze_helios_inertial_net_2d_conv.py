"""
Analyze HeliosInertialNet (2D conv + LSTM) trained by train_helios_inertial.py.

Loads models/deep_learning_models/helios_inertial_net_2d_conv.pth (full checkpoint or
state_dict), rebuilds the same train/val/test split and label encoding, evaluates on the
test set, and writes reports, figures, and predictions under analysis_results/helios_inertial_net_2d_conv/.
"""
import os
import numpy as np
import pandas as pd
...
from train_helios_inertial import (
    load_train_data,
    final_robust_split,
...
OUTPUT_DIR = "analysis_results/helios_inertial_net_2d_conv"
...
def analysis_classification_report_and_cm(all_labels, all_preds, target_names):
    ...
    with open(path_txt, "w") as f:
        f.write("Test Set Performance Analysis — Helios InertialNet (2D conv)\n")
        f.write("=" * 60 + "\n\n")
        f.write(report)
    ...
    plt.title("Confusion Matrix: Helios InertialNet (2D conv) — Gesture Classification")
...
def analysis_per_class_metrics(all_labels, all_preds, target_names):
    ...
    ax.set_xlabel("Score")
    ax.set_title("Per-Class Precision, Recall & F1 (sorted by F1) — Helios InertialNet (2D conv)")
    ax.legend()
    plt.tight_layout()
...
def analysis_confidence(all_outputs, all_labels, all_preds):
    ensure_output_dir()
...
def analysis_region_accuracy(all_labels, all_preds, target_names):
    ...
    ax.set_title("Accuracy Grouped by Body Region — Helios InertialNet (2D conv)")
...
def analysis_roc_curves(all_labels, all_probs, target_names):
    ...
    plt.suptitle("One-vs-Rest ROC Curves per Gesture Class — Helios InertialNet (2D conv)", fontsize=12, y=1.01)
    plt.tight_layout()
...
def main(checkpoint_path=CHECKPOINT_PATH):
    ...
    model, test_loader, target_names, _, _ = load_model_and_data(checkpoint_path)
    ...
    analysis_confidence(all_outputs, all_labels, all_preds)
